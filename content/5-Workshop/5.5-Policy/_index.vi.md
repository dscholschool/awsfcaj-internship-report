...
Category của sản phẩm phải khớp với loại file: category thuộc nhóm **DOCUMENT** nhận file tài liệu, category thuộc nhóm **MODEL_3D** nhận file mô hình 3D. Upload sai nhóm sẽ bị backend từ chối.
...
Hard-delete sản phẩm đã xuất hiện trong đơn hàng sẽ lỗi foreign key constraint trên `OrderItem`. Cách xử lý khuyến nghị là **soft delete** (cờ `isDeleted`/`isActive`) để giữ nguyên lịch sử mua trong khi sản phẩm biến mất khỏi storefront.
